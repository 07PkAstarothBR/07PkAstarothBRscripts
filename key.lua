const fs = require("fs");
const fetch = require("node-fetch");

const GITHUB_USER = "SEU_USUARIO"; // seu usuário do GitHub
const REPO = "NOME_DO_REPO";       // nome do repositório
const FILE_PATH = "key.lua";       // nome do arquivo no repo
const TOKEN = "SEU_TOKEN";         // seu token GitHub (com permissão repo)

function generateKey() {
  const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()_-+=<>?";
  let key = "";
  for (let i = 0; i < 30; i++) {
    key += chars.charAt(Math.floor(Math.random() * chars.length));
  }
  return key;
}

async function updateKey() {
  const key = generateKey();
  console.log(`🔑 Nova key gerada: ${key}`);

  // conteúdo do arquivo no GitHub
  const content = Buffer.from(`return "${key}"`).toString("base64");

  // pega o SHA atual do arquivo (necessário para atualizar)
  const res = await fetch(`https://api.github.com/repos/${GITHUB_USER}/${REPO}/contents/${FILE_PATH}`, {
    headers: {
      "Authorization": `token ${TOKEN}`,
      "Accept": "application/vnd.github.v3+json"
    }
  });
  const data = await res.json();
  const sha = data.sha;

  // atualiza arquivo no GitHub
  const update = await fetch(`https://api.github.com/repos/${GITHUB_USER}/${REPO}/contents/${FILE_PATH}`, {
    method: "PUT",
    headers: {
      "Authorization": `token ${TOKEN}`,
      "Accept": "application/vnd.github.v3+json"
    },
    body: JSON.stringify({
      message: "🔑 Atualizando key",
      content,
      sha
    })
  });

  if (update.ok) {
    console.log("✅ Key atualizada com sucesso!");
  } else {
    const err = await update.json();
    console.error("❌ Erro ao atualizar:", err);
  }
}

// primeira execução
updateKey();

// repete a cada 16h (16 * 60 * 60 * 1000 ms)
setInterval(updateKey, 16 * 60 * 60 * 1000);