...
LoginButton.Position = UDim2.new(0.1, 0, 0, 140)
...
-- ========================= CONSTRUÇÃO DO MAIN =========================
local function BuildMain(hasAdmin)
    -- Frame principal
    ...
    do
        ...
        -- Bloqueia exibição dos controles até senha correta
        local AdminBox = Instance.new("TextBox")
        AdminBox.Size = UDim2.new(0.55, 0, 0, 28)
        AdminBox.Position = UDim2.new(0, 10, 0, 44)
        AdminBox.BackgroundColor3 = Color3.fromRGB(40,40,40)
        AdminBox.Text = ""
        AdminBox.PlaceholderText = "Senha Admin"
        AdminBox.TextColor3 = Color3.fromRGB(255,255,255)
        AdminBox.Font = Enum.Font.SourceSans
        AdminBox.TextSize = 14
        AdminBox.Parent = PSecret
        local UCAdmin = Instance.new("UICorner"); UCAdmin.CornerRadius = UDim.new(0, 8); UCAdmin.Parent = AdminBox

        local UnlockBtn = Instance.new("TextButton")
        UnlockBtn.Size = UDim2.new(0, 100, 0, 28)
        UnlockBtn.Position = UDim2.new(0, 170, 0, 44)
        ...
        UnlockBtn.Parent = PSecret
        local UCUnlock = Instance.new("UICorner"); UCUnlock.CornerRadius = UDim.new(0, 8); UCUnlock.Parent = UnlockBtn

        local AdminPanel = Instance.new("Frame")
        AdminPanel.Name = "AdminPanel"
        AdminPanel.Size = UDim2.new(1, -20, 0, 70)
        AdminPanel.Position = UDim2.new(0, 10, 0, 84)
        ...
        local function UpdateInfoPanels()
            ...
            if PConf and PConf:FindFirstChild("CondMini") then
                local c = PConf.CondMini
    ...
    -- liga botões
    Sidebar:FindFirstChildWhichIsA("TextButton").MouseButton1Click:Connect(openFun) -- primeiro botão (Funções)
    for _, btn in ipairs(Sidebar:GetChildren()) do
        if btn:IsA("TextButton") then
            if btn.Text == "Temas" then btn.MouseButton1Click:Connect(openTheme) end
            if btn.Text == "Info"  then btn.MouseButton1Click:Connect(openInfo)  end
            if btn.Text == "Secret"then btn.MouseButton1Click:Connect(openSecret)end
            if btn.Text == "Config"then btn.MouseButton1Click:Connect(openConf)  end
        end
    end
